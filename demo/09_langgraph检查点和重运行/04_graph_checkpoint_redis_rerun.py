...
with RedisSaver.from_conn_string("redis://192.168.40.144:6379") as checkpointer:
    checkpointer.setup()
    # 4、编译为可执行图，执行流程
    # 4.1 编译为可执行图,并指定开始节点
    ...
    print("=================================打印检查点内容===========================================")
    result = list(graph.get_state_history(config=config))
    for state in result:
    ...
    print("=================================从指定打印点重新执行===========================================")
    # 5、获取编写测试代码的检查点，重新运行
    point_script_state = result[3]
    new_config = graph.update_state(
        point_script_state.config,
        values={"step_name": "重新编写测试代码","description":"代码有问题，需要重新编写"}
    )
    res = graph.invoke(None,config=new_config)
