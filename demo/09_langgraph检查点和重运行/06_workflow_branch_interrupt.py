...
def check_test_case2(state: TestState):
    action = interrupt("是否继续执行？[y/n]")
    if action == "y":
        return "编写测试代码"
    else:
        return "设计测试用例"
...
builder.add_conditional_edges("设计测试用例",check_test_case2)
...
# res = graph.invoke({"step_name": "开始执行", "description": "测试流程开始执行","case_status": random.randint(0,1)},config=config)
res = graph.invoke({"step_name": "开始执行", "description": "测试流程开始执行","case_status": 1},config=config)
print(res,type(res))
...
while True:
    if "__interrupt__" in res:
        ...
        value = input(prompt)
        res = graph.invoke(Command(resume= value),config=config)
    else:
        break