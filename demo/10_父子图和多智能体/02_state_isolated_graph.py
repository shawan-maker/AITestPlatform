...
def generate_test_cases(state: State1):
    ...
    chain = prompt | llm
    res = chain.invoke({"points": state["points"]})
    print("生成测试用例结果：", res.content)
    return {"test_cases": res.content}

# 生成测试点（调用子工作流实现）
def create_test_points(state: State1):
    """创建测试点"""
    print("3.1、创建测试点：")
    # 1、调用子工作流
    res = sub_graph.invoke({"input_requirement": state["requirement"]})
    # 2、获取子工作流的结果并返回给父工作流
    return {"points": res["test_points"]}

# 3.2 对主节点进行编排
main_builder = StateGraph(State1)
main_builder.add_node("create_test_points", create_test_points)
...
- 邮箱必须符合格式 `xxx@xxx.xx`"""
    ...
    res = main_graph.stream({"requirement": input_requirement},
                            subgraphs=True,
                            config=config,
