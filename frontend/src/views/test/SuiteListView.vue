      ...
      <PaginatedTable v-model:page="page" v-model:page-size="pageSize" :data="items" :loading="loading" :total="total" row-key="id" @page-change="load" @selection-change="onSelectionChange">
        ...
        <AppTableColumn actions variant="fixed" :label="t('common.actions')" :button-labels="[t('common.view'), t('page.test.stopRun'), t('common.delete')]">
          <template #default="{ row }">
            ...
            <el-button v-else-if="canEdit" link type="primary" @click="runSuiteRow(row)">{{ t('page.test.run') }}</el-button>
...
import { batchDeleteSuites, deleteSuite, listSuites } from '@/api/testManagement'
import { runSuite, getSuiteProgress, cancelRun } from '@/api/testExecution'
...
const filters = reactive({ q: '', status: '', result: '', triggered_by: '' })
const items = ref([])
...
const runningSuiteId = ref(null)
let listPolling = null
let listPollDone = false

function isRowRunning(row) {
  return runningSuiteId.value === row.id
}

async function stopSuiteRow(row) {
...
async function runSuiteRow(row) {
  try {
    var res = await runSuite(row.id)
    ElMessage.success(t('page.test.runStarted'))
    runningSuiteId.value = row.id
    activeRunId.value = res.data.data?.suite_run_id ?? res.data.data?.run_id ?? res.data.data?.id ?? null
    load()
    if (activeRunId.value) {
      if (listPolling) listPolling.stop()
      listPollDone = false
      listPolling = usePolling(async () => {
        var pRes = await getSuiteProgress(activeRunId.value)
        load(true)
...
onMounted(load)
onUnmounted(() => { if (listPolling) listPolling.stop() })
</script>
